<?php

return [
    'support' => [
        'section_heading' => 'Soporte de Cachet',
        'consider_supporting' => 'Por favor, considera apoyar Cachet a través de *GitHub Sponsors*.',
        'keep_up_to_date' => 'Mantente al día con las últimas noticias y versiones siguiendo el *blog de Cachet*.',
    ],
    'feed' => [
        'section_heading' => 'Últimas entradas del blog',
        'empty' => 'No se encontraron entradas de blog. Consulta *el blog* para más información.',
        'posted_at' => 'Publicado el :date',
    ],
    'powered_by' => 'Desarrollado por',
    'open_source_status_page' => 'La página de estado de código abierto.',
    'all_times_shown_in' => 'Todas las horas se muestran en *:timezone*.',
    'rss_feed' => 'RSS Feed de :name',
];
